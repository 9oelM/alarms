export const calculateAlarmTimes = (startTime: Date, endTime: Date, intervalInMins = 10) => {
  ...
  if (endTime.getTime() - startTime.getTime() > 0) return [startTime, endTime];
  ...
  for (let i = 1; i < numAlarmsInMiddle + 1; i++) {
    const newAlarmTime = new Date(startTime.getTime() + i * intervalInMins * SECONDS_IN_MIN * 1000);
    alarmsInMiddle.push(newAlarmTime);
  }
